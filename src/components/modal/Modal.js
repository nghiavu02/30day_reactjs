...
//createPortal
//destructoring props thành open, handleClose
const Modal = ({ open = false, handleClose = () => {} }) => {
  if (typeof document === "undefined") return <div className="modal"></div>;
  return ReactDOM.createPortal(
    <div
      className={`modal fixed inset-0 z-50 flex items-center justify-center p-5 ${open ? "" : "opacity-0 invisible"}`}
    >
      <div
        className="overlay absolute inset-0 bg-black bg-opacity-25"
        onClick={handleClose}
      ></div>
      <div className="relative modal-content w-full bg-white z-10 p-10 rounded-lg max-w-[482px]">
        <span
          className="absolute top-0 right-0 cursor-pointer w-10 h-10 bg-white rounded-full -translate-y-2/4 translate-x-2/4 flex items-center justify-center"
          onClick={handleClose}
        >
          <svg
            width="14"
        ...
        <div className="flex flex-col gap-3 mb-5">
          ...
          <input
            type="text"
            name="email"
            className="p-4 rounded-lg bg-[#E7ECF3] text-sm outline-none"
        ...
        <button className="bg-[#316BFF] w-full py-2 text-white text-[20px] font-[600] rounded-lg">
          Sign in
        </button>
      </div>
    </div>,
