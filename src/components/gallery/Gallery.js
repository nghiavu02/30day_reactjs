...
const Gallery = () => {
  const { photos, cart, favorites, handleAddCart, handleToggleFavorite } =
    useGallery();
  ...
  return (
    <div className="py-10 px-5">
      <div className="flex gap-10 font-[500] text-[16px]">
        <div className="relative text-[16px]" onClick={(e) => setShow(true)}>
          <svg
            className="relative"
            width="50"
            height="50"
          ...
          <span className="bg-[#FC2872] w-[34px] h-[34px] text-white text-[18px] rounded-full absolute top-0 right-0  translate-x-2/4 -translate-y-[10%] flex justify-center items-center">
            {cart.length}
          </span>
        </div>
      ...
      <CartList open={show} handleClose={() => setShow(false)}></CartList>
    </div>
  );
};
const GalleryItem = ({ item, handleAddCart, handleToggleFavorite }) => {
  return (
    <div className="relative cursor-pointer group">
      <img src={item?.url} alt="" className="w-full h-[500px] object-cover " />
      <button
        className="max-w-[254px] w-full rounded-lg bg-white text-[24px] font-[600] p-[20px] absolute bottom-0 right-2/4 translate-x-[50%] -translate-y-[80%] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all"
      ...
      {!item.isFavorite ? (
        <svg
          ...
          className="absolute top-0 right-0 -translate-x-[50%] translate-y-[50%] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all"
          width="50"
          height="50"
          viewBox="0 0 50 50"
          fill="none"
